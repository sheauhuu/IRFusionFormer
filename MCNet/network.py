...
from furnace.base_model import resnet50, resnet101
from furnace.seg_opr.seg_oprs import ConvBnRelu, ShareSeqConv
import numpy as np
...
if __name__ == '__main__':
    ...
    from furnace.seg_opr.loss_opr import EdgeLoss
    criterion = nn.CrossEntropyLoss(reduction='mean',
                                    ignore_index=255)  # 默认使用交叉熵损失
    edge_criterion = EdgeLoss(ignore_label=255)
    model = MCNet(2, criterion, edge_criterion)
    model.eval()
    from torchinfo import summary
    summary(model, input_size=(1, 3, 480, 480), device='cuda')
