...
class Resize:
    ...
    def __call__(self, sample:list) -> list:
        ...
        for k, v in sample.items():
            if k == 'mask':                
                sample[k] = TF.resize(v, (nH, nW), TF.InterpolationMode.NEAREST, antialias=True)
            else:
                sample[k] = TF.resize(v, (nH, nW), TF.InterpolationMode.BILINEAR, antialias=True)
        ...
        for k, v in sample.items():
            if k == 'mask':                
                sample[k] = TF.resize(v, (alignH, alignW), TF.InterpolationMode.NEAREST, antialias=True)
            else:
                sample[k] = TF.resize(v, (alignH, alignW), TF.InterpolationMode.BILINEAR, antialias=True)
...
if __name__ == '__main__':
    ...
    val_aug = get_val_augmentation([480, 480])
    sample = val_aug(sample)
    for k, v in sample.items():
        print(k, v.shape)