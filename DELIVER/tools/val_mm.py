...
def evaluate(model, dataloader, device):
    ...
    sliding = False
    i = 0
    for images, labels, name in tqdm(dataloader):
        name = name[0].split('.')[0]
        ...
        # save the pred for visualization
        preds = preds.argmax(dim=1).squeeze(1).cpu().numpy()
        preds = preds.astype(np.uint8)
        preds = preds.squeeze(0)
        # 1 -> 255
        preds = preds * 255
        cv2.imwrite(os.path.join('./infer','{}.png'.format(name)), preds)
        i += 1
    ...
    return acc, macc, f1, mf1, ious, miou, results_metrics
    ...
    for case in cases:
        ...
        if True:
            if eval_cfg['MSF']['ENABLE']:
                ...
                acc, macc, f1, mf1, ious, miou, results_metrics = evaluate(model, dataloader, device)
