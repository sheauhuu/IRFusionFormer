...
from util.util import compute_results, get_palette_MF, get_palette_PST, get_palette_KP, get_palette_IRC, visualize_pred
...
class Model_RGBT_Mask2Former(LightningModule):
    def __init__(self, cfg):
        ...
        if self.num_classes == 9 : 
            ...
            self.label_list = ["background", "crack"]
            self.palette = get_palette_IRC()
        else:
            self.label_list = ["road", "sidewalk", "building", "wall", "fence", "pole", "traffic_light", "traffic_sign",
                        "vegetation", "terrain", "sky", "person", "rider", "car", "truck", "bus", "train", "motorcycle", "bicycle"]#, "unlabeled"]W
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        self.validation_step_outputs.append(torch.tensor(conf))

        return torch.tensor(conf) 
    
    def on_validation_epoch_end(self):
        conf_total = torch.stack(self.validation_step_outputs).sum(dim=0)
        print(conf_total)
    ...
    def test_step(self, batch_data, batch_idx):
        ...
        for i in range(len(labels)):
            dice, specificity, precision, recall, accuracy, jaccard = self.evaluate_one_img(logits.argmax(1)[i].cpu().detach().numpy(), labels[i].cpu().detach().numpy())
            results_dict = {
                'dice': dice,
            ...
            self.test_step_seg.append(results_dict)
    ...
    def on_test_epoch_end(self):
        ...
        p = sum(precision_lst) / len(precision_lst)
        r = sum(recall_lst) / len(recall_lst)
        accuracy = sum(accuracy_lst) / len(accuracy_lst)
        jac = sum(jaccard_lst) / len(jaccard_lst)
        print('Dice: %.4f, Specificity: %.4f, Precision: %.4f, Recall: %.4f, Accuracy: %.4f, Jaccard: %.4f' % (dice, acc, p, r, accuracy, jac))
